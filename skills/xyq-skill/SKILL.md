...
4. **下载结果** - 将会话中生成的图片/视频批量下载到本地，自动提取 URL 并命名
...
```
1. create_session.py "用户的描述"  →  拿到 sessionId
...
- **超时**：连续轮询 3 分钟仍无结果，告知用户"生成时间较长，可稍后查看"，不再继续轮询
...
- **结果地址**：来自 `query_session` 返回的 `messages` 中 assistant 消息的 content 或结果里的视频/图片 URL。
- 在任务完成时，给出**视频/图片结果链接**。
- 过程中，只告知用户"正在生成中"。
